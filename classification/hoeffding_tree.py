...
class HoeffdingTree(BaseClassifier):
    ...
    class Node(metaclass=ABCMeta):
        ...
        def describe_subtree(self, ht, buffer, indent=0):
            ...
            class_val = max(self._observed_class_distribution,
        ...
        def describe_subtree(self, ht, buffer, indent=0):
            for branch_idx in range(self.num_children()):
                ...
                if child is not None:
                    ...
                    buffer[0] += self._split_test.describe_condition_for_branch(
                        branch_idx)
                    buffer[0] += ':\n'
    ...
    def split_criterion(self, split_criterion):
        if split_criterion != GINI_SPLIT and split_criterion != INFO_GAIN_SPLIT:
            logger.info("Invalid option {}', will use default '{}'".format(
                split_criterion, INFO_GAIN_SPLIT))
    ...
    def leaf_prediction(self, leaf_prediction):
        if leaf_prediction != MAJORITY_CLASS and leaf_prediction != NAIVE_BAYES \
            ...
            logger.info("Invalid option {}', will use default '{}'".format(
                leaf_prediction, MAJORITY_CLASS))
            ...
            if self._growth_allowed and isinstance(learning_node, self.ActiveLearningNode):
                ...
                if weight_diff >= self.grace_period:
                    self._attempt_to_split(
                        active_learning_node, found_node.parent, found_node.parent_branch)
                    active_learning_node.set_weight_seen_at_last_split_evaluation(
                        weight_seen)
    ...
    def _deactivate_learning_node(self, to_deactivate: ActiveLearningNode, parent: SplitNode, parent_branch: int):
        new_leaf = self.InactiveLearningNode(
    ...
    def _activate_learning_node(self, to_activate: InactiveLearningNode, parent: SplitNode, parent_branch: int):
        new_leaf = self._new_learning_node(
            to_activate.get_observed_class_distribution())
        if parent is None:
            self._tree_root = new_leaf
        else:
            parent.set_child(parent_branch, new_leaf)
    ...
    def get_info(self):
        ...
        description += 'memory_estimate_period: {} - '.format(
        ...
        description += 'stop_mem_management: {} - '.format(
        ...
        description += 'nominal_attributes: {} - '.format(
